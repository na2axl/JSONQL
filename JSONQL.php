<?php
    ...
    class JSONQL {
         ...
         * a folder who contains JSON files
         ...
         * in a folder who represent the DataBase.
         *
         * @return bool
         * 
         * @throws JSONQLException
        ...
        public function create_table($table_name, array $prototype) {
            ...
            if (file_exists($table_path)) {
                throw new JSONQLException('JSONQL Error: Cannot create the table "'.$table_name.'". The table file already exist.');
            }
            else {
        ...
        public function delete_database($database_name) {
            if (!file_exists($database_name)) {
                throw new JSONQLException('JSONQL Error: Cannot delete the database "'.$database_name.'". This folder doesn\'t exist.');
            }
            else {
                ...
                if (rmdir($database_name) === false) {
                    throw new JSONQLException('JSONQL Error: Cannot delete the database "'.$database_name.'". Ann error occured');
                }
                else {
                    return true;
                }
        ...
        public function delete_table($table_name) {
            $table_path = $this->database.'/'.$table_name.'.json';
            if (!file_exists($table_path)) {
                throw new JSONQLException('JSONQL Error: Cannot delete the table "'.$table_name.'". The JSON file "'.$table_path.'" doesn\'t exist.');
            }
            else {
        ...
        public function query($query) {
            ...
            switch (strtolower($query_part[0])) {
                ...
                default:
                    throw new JSONQLException('JSONQL Error: Sorry but the SQL query "'.strtoupper($query_part[0]).'" does\'nt exist or not supported in JSONQL at the moment.');
                break;
            }
        }
        ...
        public function bindValue($key, $value, $parse_method = self::PARAM_STRING) {
            if ($this->_queryIsPrepared()) {
                ...
                throw new JSONQLException('JSONQL Error: Cannot use JSONQL::bindValue() with non prepared queries. Send your query with JSONQL::prepare().');
            }
        }
         ...
         * Execute a query.
         *
         * This method execute a query only if $databse,
        ...
        public function execute() {
            ...
            if (!file_exists($table_path) || !is_readable($table_path) || !is_writable($table_path)) {
                throw new JSONQLException('JSONQL Error: Cannot execute the query. The table "'.$table.'.json" doesn\'t exists or file access is denied.');
            }
            else {
            ...
            foreach ($filters as $filter) {
                foreach ($result as $line) {
                    ...
                    switch ($filter['operator']) {
                        ...
                        default:
                            throw new JSONQLException('JSONQL Error: The operator "'.$filter['operator'].'" is not supported. Try to use one of these operators: <, <=, =, >=, >, <>.');
                        break;
                    }
                }
                $result = $temp;
        ...
        protected function _parseSelectQuery($query) {
            ...
            if (!in_array('from', $check_query_part)) {
                throw new JSONQLException('Query Parse Error : Missing the FROM parameter.');
            }
            else {
                ...
                if (in_array('where', $check_query_part)) {
                    ...
                    foreach ($keys_and_values as $num => $part) {
                        foreach ($this->operators as $operator) {
                            if (in_array($operator, explode(' ', $part))) {
        ...
        protected function _parseInsertQuery($query) {
            ...
            if (!in_array('into', $check_query_part) ||
                ...
                && !in_array('values', $check_query_part) && !in_array('value', $check_query_part))) {
                throw new JSONQLException('Query Parse Error: Missing some keywords like INTO or VALUE(s) in the query.');
            }
            else {
                $rows = array();
                $values = array();
                if (strpos($query_part[2], '(') !== false) {
                    $not_parsed_rows = explode(')', explode('(', $query)[1])[0];
                ...
                foreach (array('values', 'value') as $keyword) {
                    if (in_array($keyword, $check_query_part)) {
                        $temp = isset(explode($keyword, $query)[1]) ? explode($keyword, $query)[1] : explode(strtoupper($keyword), $query)[1];
                        break;
                    }
                }
                $index = 0;
        ...
        protected function _parseDeleteQuery($query) {
            ...
            $check_query_part = explode(' ', trim(strtolower($query)));
            if (!in_array('from', $check_query_part)) {
                throw new JSONQLException('Query Parse Error: Missing the keyword FROM in the query.');
            }
            else {
                $filters = array();
                $from_pos = array_search('from', $check_query_part);
        ...
        protected function _parseUpdateQuery($query) {
            ...
            if (!in_array('set', $check_query_part)) {
                throw new JSONQLException('Query Parse Error: Missing the keyword SET in the query.');
            }
            else {
                $filters = array();
        ...
        protected function _parseTruncateQuery($query) {
            ...
            if (count($query_part) != 2) {
                throw new JSONQLException('Query Parse Error: Malformed TRUNCATE query.');
            }
            else {
        ...
        protected function _parseValue($value) {
            ...
            elseif (trim($value, ' ')[0] == "'" && trim($value, ' ')[strlen(trim($value, ' '))-1] == "'") {
